...
**The `automation` field.** A dropdown on AI.List records automation candidacy: *empty* = not assessed (the assessment inbox — on a task still in `Open`, the signal a run died before assessing) → `candidate` / `not-candidate` (bot verdict, written only when the field is empty) → `approved` / `rejected` (human, during `/bspecs-triage`, on candidates; a human may also overturn a `not-candidate`). `rejected` is kept, not cleared — `approved / (approved + rejected)` is the bot-precision calibration signal. A future PR-drafting workflow (separate spec) consumes `approved` only; nothing in this system acts on the field. Field and option UUIDs are resolved at runtime via `GET /list/{id}/field`.
