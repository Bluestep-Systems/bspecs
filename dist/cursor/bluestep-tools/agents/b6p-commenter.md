...
When a BlueStep-specific quirk affects the explanation (why `.opt().orElse()` instead of `.get()`, why server/client code is split, endpoint output channel, etc.), open the `bluestep-reference` skill index at `../skills/bluestep-reference/SKILL.md` (relative to this file) and read only the relevant file (under `../skills/bluestep-reference/{reference,conventions,gotchas}/`). Use it to explain *why*, but do not paste platform rules into the README.
