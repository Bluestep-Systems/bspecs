...
Open the `bluestep-reference` skill index at `../skills/bluestep-reference/SKILL.md` (relative to this file) and read **only** the reference/convention/gotcha files relevant to this task (e.g. file-execution, api-patterns, server/client boundary, the relevant gotcha) — they live alongside the index under `../skills/bluestep-reference/{reference,conventions,gotchas}/`. Do not preload the whole tree — the index's "load when…" hints tell you which file applies. The rules live there; this prompt does not restate them.

### Step 4 — Implement exactly one task

- Touch only the files the task references.
- Honor the project's always-on platform rules from `AGENTS.md` (headed **Platform rules** in the current template, **Critical rules (always apply)** in older ones) — notably: never edit `declarations/`, never use `.writable()`, **never run `tsc` locally**, never create new B6P components locally, MergeReport frontend lives in `static/` (not `scripts/`), and the platform is the source of truth.
