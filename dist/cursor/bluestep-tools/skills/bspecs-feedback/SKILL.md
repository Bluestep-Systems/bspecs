...
Always capture the **plugin version** — `Read` `../../.cursor-plugin/plugin.json` and take its `version` field. Use the Read tool, not a shelled-out runtime (the POST step assumes only a shell is present). Fall back to `unknown` if the manifest is missing or unreadable; never block on it.
