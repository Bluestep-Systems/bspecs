...
For each template directly under `templates/` (relative to this file) (not the `legacy/` folder, which is a reference for the swap below and is never written), write into the **target directory**:
