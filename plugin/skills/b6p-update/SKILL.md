...
allowed-tools: Read Write Edit AskUserQuestion Bash(git:*) Bash(ls:*) Bash(cat:*) Bash(wc:*) Bash(sed:*) Bash(basename:*) Bash(test:*) Bash(diff:*) Bash(find:*)
---
...
Read `${CLAUDE_PLUGIN_ROOT}/skills/b6p-update/migrations/index.md` on every run. It is the catalogue of what a project can be behind on, and it changes from release to release — so what you migrate comes from there, not from memory.
...
1. **The tool's own list of opened directories.** Read `${CLAUDE_PLUGIN_ROOT}/skills/b6p-update/references/project-index.md` for the section covering the tool you are running in — where the list lives, how to read it safely, and how complete it is. The three supported tools store it in three unrelated shapes, and the guidance there is what keeps this step from turning into a hunt through someone's application data.
