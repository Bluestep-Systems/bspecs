...
For each template directly under `${CLAUDE_PLUGIN_ROOT}/skills/project-init/templates/` (not the `legacy/` folder, which is a reference for the swap below and is never written), write into the **target directory** chosen in step 1:
