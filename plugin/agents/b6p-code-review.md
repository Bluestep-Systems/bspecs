...
For the BlueStep-specific items, open the `bluestep-reference` skill index at `${CLAUDE_PLUGIN_ROOT}/skills/bluestep-reference/SKILL.md` and read the relevant reference/convention/gotcha file (file-execution, server/client boundary, api-patterns, the Optional/`.opt()` rules, component-library, etc. — under `${CLAUDE_PLUGIN_ROOT}/skills/bluestep-reference/{reference,conventions,gotchas}/`) rather than relying on memory. Cite the rule, don't restate the whole file.
