...
- [ ] **npm-free delivery via the VSCode extension (explore).** For shops that block npm on staff machines, fold bspecs scaffolding into the `b6p-vscode` extension (which is already npm-free — it bundles `b6p-core`, not `b6p-cli`). Mechanically easy; the real work is three decisions: a shared `bspecs-core` template source-of-truth, an npm-free *profile* of the scaffolded content (b6p skills calling extension commands instead of `npx b6p`), and prompt UX. The binding constraint is a security policy against arbitrary code execution (`npm install` runs arbitrary code), which **rejects** the internal-registry option and **promotes** single-artifact delivery — moving the code-execution surface to one audited build pipeline. Claude Code host + b6p engine are already npm-free (extension/desktop app + bundled `b6p-core`), so the only remaining npm surface is the scaffolded content. Feasibility analysis + recommendation captured in [`docs/decisions/npm-free-scaffolding-via-vscode-extension.md`](docs/decisions/npm-free-scaffolding-via-vscode-extension.md) (Proposed — leaning accept).
