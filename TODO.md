...
- [ ] **`/bspecs-feedback` should also create a linked ClickUp task.** In addition to the prefilled GitHub issue, the skill should create a ClickUp task — tagged `ai-plugin` — on the [`ai-plugin` ClickUp space](https://app.clickup.com/1282031/v/l/li/901414350506), linked back to the filed GitHub issue. **Open question to resolve before implementing:** the skill's whole design ([`docs/decisions/bspecs-feedback-mechanism.md`](docs/decisions/bspecs-feedback-mechanism.md)) deliberately avoids any token/backend so it works for *every* user via browser-session auth against a **public** repo — ClickUp task creation needs either a baked-in API token (the exact secret-in-scaffold pattern that ADR rejected as Option B) or the user being a member of that specific ClickUp space, which not every bspecs user has. Need a path that still works (or degrades gracefully, e.g. falling back to just the GitHub issue) for users without access to that space before wiring this in.
