...
- [x] **Auto-create a linked ClickUp task when a `feedback` issue is filed.** Shipped as `.github/workflows/feedback-to-clickup.yml` (triggers on `issues: opened`, guarded by the `feedback` label; creates the `ai-plugin`-tagged task via `CLICKUP_API_TOKEN` repo secret and comments the task URL back on the issue). Add a GitHub Action (triggered on `issues: opened`, scoped to the `feedback` label) that calls the ClickUp API **server-side** — using a repo-level Actions secret (e.g. `CLICKUP_API_TOKEN`), never shipped to users or the plugin — to create a task tagged `ai-plugin` on the [`ai-plugin` ClickUp space](https://app.clickup.com/1282031/v/l/li/901414350506), then comments the ClickUp task URL back on the GitHub issue to link the two. This sidesteps the access problem a client-side/skill-side implementation would have: not every bspecs user is a member of that ClickUp space, and baking a token into the scaffold was already rejected (see [`docs/decisions/bspecs-feedback-mechanism.md`](docs/decisions/bspecs-feedback-mechanism.md) Option B) — here the secret lives only in GitHub's encrypted store and the workflow runs in this repo's CI, not on any user's machine, so no user-side ClickUp auth is needed at all. `/bspecs-feedback` itself is unchanged — it still just files the GitHub issue. Needs someone to own/rotate the ClickUp API token in repo secrets; keep the trigger scoped to the `feedback` label so it doesn't fire on unrelated repo issues.
