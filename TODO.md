...
- [ ] **b6p-cli onboarding in scaffolds — first-run auth & download duplication.** On a machine that never ran `b6p auth set`, the first `npx b6p pull` hangs on an interactive credential prompt Claude can't answer (`--yes` guards the *confirmation* prompt, not the *credentials* one). The b6p skills only mention auth in error-handling, with no preflight, and the one-time `b6p auth set` step lives in the scaffolded README prose, not in `CLAUDE.md` / skill preconditions. Likely fix: an auth-preflight in the b6p skills + surface `auth set` where the agent reads it. Also notes the (accepted, A5) per-project download duplication. Credentials are global (`~/.b6p`), so there is **no** re-login per scaffold. Discussion + options in [`docs/decisions/b6p-cli-onboarding-in-scaffolds.md`](docs/decisions/b6p-cli-onboarding-in-scaffolds.md). Needs its own spec/bug.
