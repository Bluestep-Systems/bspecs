...
- [ ] **Reduce install friction — registry decision (awaiting BlueStep).** The GitHub PAT is mandatory because GitHub Packages requires auth for every install, even public packages — it gates `bspecs` itself, so no `bspecs init` can bootstrap it. Removing the PAT requires publishing to the public npm registry, which is BlueStep's call (and `b6p-cli`'s is the `Bluestep-Systems/vscode-extension` team's). Options + the question to ask BlueStep are in [`docs/decisions/install-friction-and-registry.md`](docs/decisions/install-friction-and-registry.md) (status: Proposed). Decide between: stay private, fully public, or public-bspecs/private-b6p.
- [ ] **Auto-run `npm install` in the scaffolded project.** Flip `scaffold.js:reportInstallStep` from printing a reminder to actually running `npm install`, so the user doesn't run it by hand. The original "avoid a PAT at scaffold time" reason is stale — the user already has the token configured by then. Independent of the registry decision above. See [`docs/decisions/install-friction-and-registry.md`](docs/decisions/install-friction-and-registry.md).
- [ ] **`bspecs doctor` / `bspecs init` command.** Check Node version, write/validate the `~/.npmrc` scope mapping, and tell the user which PAT scope to create if the token is missing (it can't mint the token). Turns three README steps into one command. Independent of the registry decision. See [`docs/decisions/install-friction-and-registry.md`](docs/decisions/install-friction-and-registry.md).
