...
- [ ] **Phase 4 — automate `[PLATFORM]` tasks (the actual payoff — go straight here).** Teach `/spec-execute` (or a new subagent) to run `add_queries` / `add_forms` / `add_field_access` + the schema-authoring tools for `[PLATFORM]`-tagged tasks, approval-gated, turning today's human hand-back into an in-session action. After wiring, read declarations back via `get_script_declarations` — prove-out bar (weak, not byte-parity): the output is **sufficient to code against**; the ambient typedefs come from the CLI-pulled tree or a bundled static set. Prereq still open: **confirm in-session tool registration works in-app** (only the curl `initialize` handshake was verified previously; a fresh live session is required — a newly-added connection does not hot-load into a running session). Spec in progress: `.claude/specs/mcp-platform-authoring/`. **Implementation shipped in plugin `0.8.0`** via `.claude/specs/mcp-platform-authoring/` (Tasks 1–8): the shared procedure page + all three entry points wired (`/spec-execute`, `/quick-task`, scaffolded `CLAUDE.md`) + the tasks/`CLAUDE.md` templates + the test plan. **Remaining gate before ticking:** the live prove-out (Task 9 / `docs/mcp-platform-authoring-test-plan.md`) confirming in-app tool registration + create/assert/teardown on bkplayground.
- [ ] **Update the out-of-repo global `~/.claude/CLAUDE.md` rule to match.** The global B6P rule ("new imports require a platform round-trip → hand back to a human") predates Phase 4 and should be reworded to "perform platform authoring/wiring in-session via MCP when connected, else hand back." That file lives **outside this repo** (the user's home dir), so it cannot be edited here — this is a reminder to update it manually.
- [ ] **Phase 5 — fold into skills + `bluestep-reference` + the scaffolded `CLAUDE.md`, then release** (tag the then-current `plugin-vX.Y.Z`). Scope is the Phase-4 `[PLATFORM]` authoring flow only (not a sync migration).

> **Release-tag note.** The tag was deferred "until the migration lands" = the *pull/push* migration, which is now off the table. The connection (`0.7.0`) is complete and useful standalone; whether to cut a `0.7.0` tag now or gate the tag on Phase 4 landing is still open — pending a call.
