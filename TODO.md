...
## Gateway MCP — collapse per-org connections into one global endpoint

- [x] **Test the global gateway MCP and, if it works, replace the per-org connect flow.** A new global MCP at `https://gateway.bluestep.net/mcp` is meant to expose all orgs a user is allowed to connect to from a single endpoint — collapsing today's one-`bluestep-<subdomain>`-entry-per-org model (see [`docs/decisions/platform-mcp-integration.md`](docs/decisions/platform-mcp-integration.md)). Plan: (1) **verify** the gateway authenticates with the existing global `b6pt_` token and actually surfaces every reachable org's tools; (2) if it works, **ship the gateway as a plugin-level MCP tool** (bundled in the plugin, not per-org conversational setup); (3) **retire `/bluestep-mcp-connect`** and **remove the MCP-connection step from `/bluestep-init`**; (4) amend the platform-MCP ADR to record the topology change. Tracked as spec `gateway-mcp` under `.claude/specs/`.
