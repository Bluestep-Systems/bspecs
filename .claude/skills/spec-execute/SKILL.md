...
4. **Implement exactly one task.** No scope creep:
   ...
   - Apply the conventions in `CLAUDE.md` (template variables, `.template` stripping, `mirrorInstructionsToGithub` sync, English-only committed files).
...
7. **Keep docs in sync.** If this task changed behavior described in `CLAUDE.md` or `README.md`, or completed a `TODO.md` item, update them in the same change. If an instruction template was edited, confirm the `.github/instructions/` mirror is accounted for.
