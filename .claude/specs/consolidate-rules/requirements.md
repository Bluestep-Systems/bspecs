...
- [ ] `SYNC_TARGETS` (in `src/sync.js`) is derived dynamically by walking the whole `templates/claude/**` tree (skills, hooks, settings, spec-templates, and the new instructions tree), with a documented `SYNC_EXCLUDE` escape hatch — no hardcoded per-file array. `bspecs sync` and `bspecs.lock` track every file automatically, and the prior hardcoded set resolves to identical destinations.
