...
- [x] **1.** **Wire b6p into scaffolded projects.** Ensure scaffolded output has
...
- [x] **2.** **Decide + implement install step.** Either run `npm install` in the scaffolded project
...
- [x] **3.** **Switch `/b6p-pull` to `npx b6p`.** Replace all `shellPrefix` / `wsl <shell> -lc`
      invocations with `npx b6p …`; remove "command not found → install" guidance. — files:
      `templates/claude/skills/b6p-pull/SKILL.md`

- [x] **4.** **Switch `/b6p-push` and `/b6p-audit` to `npx b6p`.** Same mechanical change as task 3.
      — files: `templates/claude/skills/b6p-push/SKILL.md`,
      `templates/claude/skills/b6p-audit/SKILL.md`

- [x] **5.** **Remove / reduce the WSL hook.** Per the resolved open question: delete
...
- [x] **6.** **Delete the `/b6p-detect` skill.** Remove the folder; confirm no skill/doc references
      it. (Dynamic `SYNC_TARGETS` drops it automatically.) — files:
      `templates/claude/skills/b6p-detect/` (delete)

- [x] **7.** **Strip detection from `src/scaffold.js`.** Remove `detectEnvironmentFor`,
...
- [x] **12.** **End-to-end verify.** `node cli.js` scaffold into a scratch dir; `npm install` in the
